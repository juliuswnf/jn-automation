  ...
  return (
    <div className="min-h-screen bg-primary text-white px-4 py-8">
      <div className="max-w-2xl mx-auto">
        ...
        {sessions.length === 0 ? (
          <div className="rounded-lg bg-secondary/50 border border-accent/20 p-8 text-center">
            <p className="text-slate-350">No active sessions</p>
          </div>
        ) : (
          <div className="space-y-4">
            {sessions.map((session) => (
              <div
                key={session.id}
                className="rounded-lg bg-secondary/50 border border-accent/20 p-6 hover:border-accent/40 transition"
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  {session.isCurrent && (
                    <span className="px-3 py-1 rounded-full bg-accent/10 border border-accent text-accent text-xs font-semibold">
                      Current
                    </span>
                  )}
                </div>
                ...
                {!session.isCurrent && (
                  <button
                    ...
                    className="w-full px-4 py-2 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 hover:bg-red-500/20 hover:border-red-500/50 font-semibold transition disabled:opacity-50"
                  >
