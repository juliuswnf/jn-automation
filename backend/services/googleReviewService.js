...
export const sendReviewRequestEmail = async (booking) => {
  try {
    ...
    if (!template) {
      logger.error(`âŒ Review email template not found for language: ${booking.language}`);
    ...
    logger.error('âŒ SendReviewRequestEmail Error:', error);
    throw error;
  }
};
  ...
  try {
    ...
    for (const booking of bookings) {
      try {
        ...
        logger.error(`âŒ Failed to send review email for booking ${booking._id}:`, error);
      }
    }
    ...
    logger.error('âŒ ProcessCompletedBookings Error:', error);
    throw error;
  }
};
