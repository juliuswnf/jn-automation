...
export const getCollectionCounts = async () => {
  try {
    ...
    logger.error('âŒ Error getting collection counts:', error.message);
    throw error;
  }
};
...
export const showDatabaseStatus = async () => {
  try {
    ...
    logger.log(`   ðŸ“ˆ TOTAL DOCUMENTS: ${total}\n`);

    return total;
  } catch (error) {
    logger.error('âŒ Error showing database status:', error.message);
    throw error;
  }
};
...
export const clearAllCollections = async () => {
  try {
    logger.log('ðŸ—‘ï¸  Clearing all collections...\n');
    ...
    for (const collection of collections) {
      ...
      logger.log(`   âœ… ${collection.name}: ${result.deletedCount} documents deleted`);
      totalDeleted += result.deletedCount;
    }

    logger.log(`\nâœ… Total ${totalDeleted} documents deleted\n`);
    return totalDeleted;
  } catch (error) {
    logger.error('âŒ Error clearing collections:', error.message, '\n');
    throw error;
  }
};
...
export const clearCollection = async (collectionName) => {
  try {
    ...
    logger.log(`\nâœ… Cleared ${collectionName}: ${result.deletedCount} documents deleted\n`);
    return result.deletedCount;
  } catch (error) {
    logger.error('âŒ Error clearing collection:', error.message, '\n');
    throw error;
  }
};
...
export const dropDatabase = async () => {
  try {
    ...
    logger.error('âŒ Error dropping database:', error.message, '\n');
    throw error;
  }
};
...
export const backupBeforeClear = async () => {
  try {
    ...
    logger.log(`âœ… Backup created: ${filePath}\n`);

    return filePath;
  } catch (error) {
    logger.error('âŒ Error creating backup:', error.message);
    throw error;
  }
};
...
const interactiveMode = async () => {
  try {
    logger.log('================================');
    logger.log('  ðŸ—‘ï¸  DATABASE CLEANUP TOOL');
    logger.log('================================\n');
    ...
    if (option === '1') {
      ...
      const confirmDrop = await question('âš ï¸  Are you sure? This will drop the entire database (yes/no): ');
...
const handleCommandLineArgs = async () => {
  ...
  try {
    ...
    if (command === '--status') {
      ...
      if (!confirm) {
        logger.log('âš ï¸  Use --force flag to confirm: npm run clear -- --clear-all --force');
        process.exit(0);
      }
      await clearAllCollections();
      logger.log('âœ… Database cleared\n');
      process.exit(0);
      ...
      const confirm = args[1] === '--force';
      if (!confirm) {
        logger.log('âš ï¸  Use --force flag to confirm: npm run clear -- --drop --force');
        process.exit(0);
      }
      await dropDatabase();
      logger.log('âœ… Database dropped\n');
      process.exit(0);
