...
async function checkSPF() {
  console.log(`\n${c.cyan}â•â•â• SPF Record Check â•â•â•${c.reset}`);
  ...
  try {
    ...
    if (spfRecord) {
      console.log(`${c.green}âœ“ SPF Record found:${c.reset}`);
      console.log(`  ${c.dim}${spfRecord}${c.reset}`);

      // Check for common issues
      if (spfRecord.includes('-all')) {
        addResult('DNS', 'SPF Record', 'pass', 'Strict policy (-all)');
        console.log(`  ${c.green}âœ“ Strict policy (-all) - Excellent!${c.reset}`);
      } else if (spfRecord.includes('~all')) {
        addResult('DNS', 'SPF Record', 'pass', 'Soft fail policy (~all)');
        console.log(`  ${c.yellow}âš  Soft fail policy (~all) - OK but consider -all${c.reset}`);
      } else if (spfRecord.includes('?all')) {
        addResult('DNS', 'SPF Record', 'warn', 'Neutral policy (?all)');
        console.log(`  ${c.yellow}âš  Neutral policy (?all) - Should use ~all or -all${c.reset}`);
      } else if (spfRecord.includes('+all')) {
        addResult('DNS', 'SPF Record', 'fail', 'Permissive policy (+all)');
        console.log(`  ${c.red}âœ— Permissive policy (+all) - INSECURE!${c.reset}`);
      }
      ...
      if (EMAIL_CONFIG.host && spfRecord.toLowerCase().includes(EMAIL_CONFIG.host.split('.').slice(-2).join('.'))) {
        console.log(`  ${c.green}âœ“ SMTP provider likely included in SPF${c.reset}`);
      }

      return true;
    } else {
      addResult('DNS', 'SPF Record', 'fail', 'No SPF record found');
      console.log(`${c.red}âœ— No SPF record found!${c.reset}`);
    ...
    if (error.code === 'ENOTFOUND' || error.code === 'ENODATA') {
      ...
      console.log(`${c.red}âœ— No TXT records found for domain${c.reset}`);
    } else {
...
async function checkDKIM() {
  console.log(`\n${c.cyan}â•â•â• DKIM Record Check â•â•â•${c.reset}`);
  ...
  for (const selector of selectors) {
    ...
    try {
      ...
      if (dkimRecord.includes('v=DKIM1') || dkimRecord.includes('p=')) {
        console.log(`${c.green}âœ“ DKIM Record found (selector: ${selector}):${c.reset}`);
  ...
  if (!foundDKIM) {
    ...
    console.log(`${c.yellow}âš  No DKIM record found with common selectors${c.reset}`);
    ...
    console.log(`  If using Gmail Workspace: Admin Console â†’ Apps â†’ Google Workspace â†’ Gmail â†’ Authenticate email`);
  }
...
async function checkDMARC() {
  console.log(`\n${c.cyan}â•â•â• DMARC Record Check â•â•â•${c.reset}`);
  ...
  try {
    ...
    if (dmarcRecord) {
      console.log(`${c.green}âœ“ DMARC Record found:${c.reset}`);
      ...
      if (policyMatch) {
        ...
        if (policy === 'reject') {
          ...
          console.log(`  ${c.green}âœ“ Policy: reject - Maximum protection${c.reset}`);
        } else if (policy === 'quarantine') {
          addResult('DNS', 'DMARC Record', 'pass', 'Policy: quarantine');
          console.log(`  ${c.green}âœ“ Policy: quarantine - Good protection${c.reset}`);
        } else if (policy === 'none') {
          addResult('DNS', 'DMARC Record', 'warn', 'Policy: none (monitoring only)');
          console.log(`  ${c.yellow}âš  Policy: none - Monitoring only, no protection${c.reset}`);
        }
      }

      // Check for reporting
      if (dmarcRecord.includes('rua=')) {
        console.log(`  ${c.green}âœ“ Aggregate reporting configured${c.reset}`);
      }

      return true;
    } else {
      addResult('DNS', 'DMARC Record', 'fail', 'No DMARC record found');
      console.log(`${c.red}âœ— No DMARC record found!${c.reset}`);
...
async function checkMX() {
  console.log(`\n${c.cyan}â•â•â• MX Record Check â•â•â•${c.reset}`);

  try {
    const records = await resolveMx(FROM_DOMAIN);

    if (records && records.length > 0) {
      console.log(`${c.green}âœ“ MX Records found:${c.reset}`);
      ...
      console.log(`${c.red}âœ— No MX records found${c.reset}`);
      return false;
    }
  } catch (error) {
...
async function testSMTPConnection() {
  console.log(`\n${c.cyan}â•â•â• SMTP Connection Test â•â•â•${c.reset}`);
  ...
  if (!EMAIL_CONFIG.host || !EMAIL_CONFIG.user || !EMAIL_CONFIG.pass) {
    ...
    console.log(`\n${c.red}âœ— SMTP not fully configured!${c.reset}`);
  ...
  try {
    ...
    console.log(`${c.green}âœ“ SMTP connection successful!${c.reset}`);
    return transporter;
  } catch (error) {
    addResult('SMTP', 'Connection', 'fail', error.message);
    console.log(`${c.red}âœ— SMTP connection failed: ${error.message}${c.reset}`);
...
async function sendTestEmail(transporter, recipientEmail) {
  console.log(`\n${c.cyan}â•â•â• Sending Test Email â•â•â•${c.reset}`);
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f8fafc;">
  ...
  <div style="background: white; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    ...
    <ol style="color: #4b5563;">
      <li>SPF Record fÃ¼r ${FROM_DOMAIN} prÃ¼fen</li>
      <li>DKIM Konfiguration Ã¼berprÃ¼fen</li>
      <li>DMARC Policy checken</li>
      <li>Absender zu Kontakten hinzufÃ¼gen</li>
    </ol>
  ...
  try {
    ...
    console.log(`${c.green}âœ“ Test email sent successfully!${c.reset}`);
    console.log(`  Message ID: ${info.messageId}`);
    console.log(`  Test ID: ${testId}`);
    addResult('Email', `Test to ${recipientEmail}`, 'pass', `Message ID: ${info.messageId}`);
    return true;
  } catch (error) {
    console.log(`${c.red}âœ— Failed to send: ${error.message}${c.reset}`);
...
function showMailTesterInstructions() {
  console.log(`\n${c.cyan}â•â•â• mail-tester.com Spam Score Check â•â•â•${c.reset}`);
  console.log(`
${c.bold}Schritt-fÃ¼r-Schritt Anleitung:${c.reset}

1. Ã–ffne ${c.blue}https://www.mail-tester.com${c.reset}

2. Du siehst eine Test-E-Mail-Adresse wie:
   ${c.dim}test-xyz123@srv1.mail-tester.com${c.reset}

3. FÃ¼hre dieses Skript erneut aus mit dieser Adresse:
...
${c.bold}Bewertung verstehen:${c.reset}
  ...
  ${c.green}8-9/10${c.reset} - Sehr gut, minor improvements mÃ¶glich
  ${c.yellow}6-7/10${c.reset} - OK, aber SPF/DKIM/DMARC prÃ¼fen
  ${c.red}< 6/10${c.reset} - Kritisch! Viele E-Mails landen im Spam

${c.bold}HÃ¤ufige Probleme & Fixes:${c.reset}
  â€¢ Missing SPF â†’ DNS TXT Record hinzufÃ¼gen
  â€¢ Missing DKIM â†’ Bei E-Mail Provider aktivieren
  â€¢ Missing DMARC â†’ DNS TXT Record hinzufÃ¼gen
  â€¢ Blacklisted IP â†’ Anderen SMTP Provider nutzen
  â€¢ HTML Issues â†’ Templates vereinfachen
`);
}
...
function printSummary() {
  ...
  Object.entries(categories).forEach(([category, tests]) => {
    ...
    tests.forEach(t => {
      const icon = t.status === 'pass' ? `${c.green}âœ“` : t.status === 'fail' ? `${c.red}âœ—` : `${c.yellow}âš `;
  ...
  if (results.failed > 0) {
    console.log(`\n${c.red}${c.bold}âš  KRITISCHE PROBLEME GEFUNDEN!${c.reset}`);
    console.log(`${c.red}Ohne Fixes landen 60-70% der E-Mails im Spam.${c.reset}\n`);

    console.log(`${c.bold}Empfohlene DNS Records fÃ¼r ${FROM_DOMAIN}:${c.reset}`);
    console.log(`
...
${c.cyan}DKIM:${c.reset}
  â†’ Wird vom E-Mail Provider konfiguriert (Gmail, Outlook, etc.)
  â†’ Google Workspace: Admin â†’ Apps â†’ Gmail â†’ Authenticate email
`);
  } else if (results.warnings > 0) {
    console.log(`\n${c.yellow}âš  Einige Verbesserungen empfohlen, aber kein kritisches Problem.${c.reset}`);
  } else {
    console.log(`\n${c.green}âœ“ Alle Tests bestanden! E-Mail Deliverability sieht gut aus.${c.reset}`);
  }
}
...
async function main() {
  console.log(`\n${c.bold}${c.blue}â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—${c.reset}`);
  console.log(`${c.bold}${c.blue}â•‘        JN AUTOMATION - EMAIL DELIVERABILITY TEST           â•‘${c.reset}`);
  console.log(`${c.bold}${c.blue}â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•${c.reset}\n`);
  ...
  if (testEmail && transporter) {
    ...
    if (testEmail.includes('@gmail')) {
      console.log(`\n${c.yellow}Gmail Hinweis:${c.reset} PrÃ¼fe den Spam-Ordner und klicke "Kein Spam", um zukÃ¼nftige E-Mails im Inbox zu erhalten.`);
    } else if (testEmail.includes('@outlook') || testEmail.includes('@hotmail')) {
      console.log(`\n${c.yellow}Outlook Hinweis:${c.reset} PrÃ¼fe den Junk-Ordner und fÃ¼ge den Absender zu "Sichere Absender" hinzu.`);
    } else if (testEmail.includes('@web.de') || testEmail.includes('@gmx')) {
      console.log(`\n${c.yellow}Web.de/GMX Hinweis:${c.reset} PrÃ¼fe den Spam-Ordner und fÃ¼ge den Absender zum Adressbuch hinzu.`);
    }
  } else if (!testEmail) {
    console.log(`\n${c.yellow}Tipp: FÃ¼hre mit E-Mail-Adresse aus, um eine Test-E-Mail zu senden:${c.reset}`);
